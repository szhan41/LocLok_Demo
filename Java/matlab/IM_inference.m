function [pr_post] = IM_inference(p_prior, z, DeltaX, eps, T, A, vertices)
...
% Trans_A === A in IM_Release
    ...
    [d,N]=size(T);
    if(size(DeltaX,1)~=1 )%if only one row, then stay the same. i.e., sens=0;
    ...
    for i=1:N
        if pr(1,i)==1 
            pr(1,i)=exp(- eps *  NormOfK( A*(z -Delta(:,col)),  Trans_vertices)      );
            col=col+1;
        end
    end
